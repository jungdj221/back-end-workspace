/*
    ...
    - 데이터 조작 언어로 테이블 값을 삽입(INSERT), 수정(UPDATE)하거나, 삭제(DELETE)하는 구문
    
    INSERT
    - 테이블 새로운 행을 추가하는 구문
    
    1) INSERT INTO 테이블명 VALUES (값, 값, 값...);
		- 테이블의 모든 컬럼에 대한 값을 INSERT 하고자 할떄 사용.
        - 컬럼 순번을 지켜서 VALUES에 값을 나열해야 한다
    2) INSERT INTO 테이블명(컬럼명, 컬럼명,...) VALUES(값,값,...)
		- 테이블의 특정 컬럼에 대한 값만 INSERT 하고자 할때 사용한다.
...
INSERT INTO emp VALUES (200, '유지혁', '개발 지원팀'); -- 모든 컬럼값을 지정하지 않아서 에러 발생
INSERT INTO emp VALUES (300, '유아지원팀', '박민환', default); -- 에러는 발생한하지만, 데이터가 잘못 저장
INSERT INTO emp VALUES ('유아지원팀', '400', '정수민', default); -- 데이터타입이 맞지 않음

-- 2) 
INSERT INTO emp(emp_id, emp_name, dept_title, hire_date) VALUES (400, '백은호', '인사팀', null);
INSERT INTO emp(emp_name, dept_title, emp_id) VALUES ('양주란', '보안팀', 500); -- 컬럼명과 값이 위치만 동일시키면 됨.
INSERT INTO emp(emp_id, emp_name) VALUES(600,'유희연');
INSERT INTO emp(emp_id, dept_title) VALUES(700, '마케팅팀'); -- emp_name 컬럼에 not null 제약조건으로 인해 에러 발생

-- 3) 
-- kh.employee 테이블에서 사번, 이름, 부서명, 입사일을 그대로 가지고 옴.
...
INSERT INTO emp(emp_name,emp_id,dept_title,hire_date) -- 컬럼명을 명시하면 순서는 상관없음
...
/*
    ...
    - SET 절에서 여러 개의 컬럼을 콤마(,)로 나열해서 값으 동시에 변경할 수 있다.
    - WHERE 절을 생략하면 모든 행의 데이터가 변경된다. (MySQL 방지)
    - 사실 서브쿼리 사용 가능. 잘쓰이지도 않고, 버전마다 상황이 다름
*/
...
-- 모든 사원의 급여를 기존 급여에서 10프로 인상 금액(기존 급여 *1.1)
UPDATE emp_salary
...
UPDATE dept_salary
SET emp_id = null
WHERE emp_id = 200; -- 해당 컬럼에 대한 제역조건이 위배되면 수정 x
...
rollback; -- truncate로 행을 삭제하면 rollback 불가능
