...
AS SELECT * FROM kh.department; -- 서브쿼리를 실제 쓸일은 없음
...
-- > 새로운 컬럼이 만들어지고 기본적으로 NULL 채워짐

-- LNAME 컬럼 추가 (기본값 : 한국)
ALTER TABLE dept_copy ADD lname VARCHAR(20) DEFAULT '한국';
-- > 새로운 컬림이 만들어지고 지정한 기본값으로 채워짐
SELECT * FROM dept_copy;
...
ALTER TABLE dept_copy MODIFY dept_id INT; -- 오류
...
-- > 변경하려는 자료형의 크기보다 이미 큰 값이 존재하면 애러 발생
-- dept_title 컬럼의 자료형은 VARCHAR(40),
-- location_ id 컬럼의 자료형은 VARCHAR(2),
-- lname 컬럼의 기본값을 미국을 변경
ALTER TABLE dept_copy 
	MODIFY dept_title VARCHAR(40),
	MODIFY location_id VARCHAR(2),
	MODIFY lname VARCHAR(10) DEFAULT '미국'; -- null 값이 미국으로 치환됨.
...
/*
    ...
    - ADD CONSTRAINT 제약조건명 UIQUE(컬럼명);
...
-- dept_copy 태이블의 dept_id 컬럼에 기본키(PK, PRIMARY KEY) 제약조건 추가
...
ALTER TABLE dept_code ADD UNIQUE(dept_title);
...
-- 만약 삭제하고 싶다면 자식테이블 먼저 삭제 후 부모테이블 삭제
DROP TABLE member;
DROP TABLE member_grade;
...
CREATE TABLE member(
    ...
    CONSTRAINT member_grade_id_fk FOREIGN KEY (grade_code) REFERENCES member_grade(grade_code)
);
