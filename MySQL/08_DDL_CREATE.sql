/*
    ...
    - 객체 (Object)/스키마(Schemaa)를 만들고(CREATE), 변경하고(ALTER), 삭제(DROP)하는 언어
    - 실제 데이터 값이 아닌 구조 자체를 정의하는 언어
    - 주로 DB 관리자, 설계자가 사용
    
    * MySQL에서 객제(구조) : 테이블(Table), 뷰(View), 인덱스(Index), 프로시저(Prodecure), 트리거(Trigger), 함수(Function)
*/
/*
	CREATE
    - 객체를 생성하는 구문
...
-- 테이블의 데이터를 추가시키는 구문 (DML : INSERT)
-- INSERT INTO 테이블명 VALUES(값,값,...)
INSERT INTO member VALUES(1, 'user01','pass01','이상현', '남', '010-0000-0000', 'aaa@gmail.com','2023-11-30');
INSERT INTO member VALUES(2, 'user02','pass02','이상호', '남', null, null, NOW());
INSERT INTO member VALUES(3, 'user03','pass03','이상호', '남', null, null, current_date());
INSERT INTO member VALUES(null, null ,null ,null ,null ,null ,null ,null);

/*
	제약조건(CONSTRAINTS)
    - 사용자가 원하는 조건의 데이터만 유지하기 위해서 각 컬럼에 대해 저장될 값에 대한 제약조건을 설정
    - 제약조선은 데이터 무결성 보장을 목적으로 한다.
    - 종류 : NOT NULL, UNIQUE, CHECK, PRIMIARY KEY, FOREIGN KEY
*/
/*
	NOT NULL
    - 해당 컬럼에 반드시 값이 있어야만 하는 경우(해당 컬럼에 절대 NULL이 들어와서는 안되는 경우)
...
INSERT INTO mem_check VALUES(3, 'user03', 'pass03', '손민정',null, null, null, null);
-- > check 제약조선뿐만 아니라 null도 처리해야됨
...
-- > 회원번호(mem_no)가 동일해도 insert가 되는 문제

SELECT * FROM mem_check;

/*
	PRIMARY kEY (기본키)
    - 테이블에서 각 행들을 식별하기 위해 사용될 컬럼에 부여하는 제약조건 (식별자)
    ex) 회원번호, 학번, 사원번호, 부서코드, 직급코드, 제품번호, 주문번호, 운송장번호...
    - PRIMARY KEY 제약조건을 부여하면 그 컬렘에 자동으로 NOT NULL + UNIQUE 조건이 설정
    - 한 테이블 당 오로지 한 개만 설정
    - 복합키 설정도 가능
*/
DROP TABLE mem_pri;
CREATE TABLE mem_pri(
    ...
    mem_pwd VARCHAR(20) NOT NULL,
    mem_name VARCHAR(20) NOT NULL,
    gender CHAR(3) NOT NULL CHECK (gender IN('남','여')) NOT NULL,
    phone VARCHAR(13),
    email VARCHAR(50),
    mem_date DATE,
...
-- > 기본 키의 중복값을 담으려고 해서 문제 발생 = unique 제약조건 위배
-- > 기본키를 mem_no + mem_id 복합키로 설정했을 때는 가능!
INSERT INTO mem_pri VALUES(null, 'user02', 'pass02', '조세미', '여', null, null, null);
-- > 기본 키에 null을 담으려고 해서 문제 발생 (not null 제약조건 위배)
INSERT INTO mem_pri VALUES(2, 'user02', 'pass02', '윤민영', '여', null, null, null);
...
/*
    ...
    - 다른 테이블에 존재하는 값만 드러와야 되는 특정 컬럼에 부여하는 제약조건
	  (단, NULL 값은 가질 수 있음)
...
DROP TABLE member;
CREATE TABLE member(
	mem_no INT, -- PRIMARY KEY, -- 컬럼 레벨방식
    mem_id VARCHAR(20) NOT NULL UNIQUE,
    mem_pwd VARCHAR(20) NOT NULL,
    mem_name VARCHAR(20) NOT NULL,
    gender CHAR(3) NOT NULL CHECK (gender IN('남','여')) NOT NULL,
...
/*
	자식테이블 생성시 외래키 제약조건을 부여할때 삭제옵션 지정 가능
    - 삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지
...
-- mem_grade에서 grade_code가 10인 등급 삭제
DELETE FROM mem_grade WHERE grade_code = 10; -- ON DELETE SET NULL
-- > 잘 삭제됨. 단, 10일 가져다쓰고있던 자식 데이터 값은 NULL로 변경
...
DROP TABLE member;
CREATE TABLE member(
	mem_no INT AUTO_INCREMENT PRIMARY KEY, -- 컬럼 레벨방식
    mem_name VARCHAR(20) NOT NULL,
    mem_age INT,
...
-- INSERT INTO 테이블명(칼럼명, 컬렴명....)  VALUES(컬럼값, 컬럼값...);
...
/*
    ...
    AS 서브쿼리;
*/
-- employee 테이블 복사하여 새로운 테이블 생성(컬럼, 데이터 타입, not null 제약조건 다 복사)
...
-- 모든행에 대해서 매번 false이기 때문에 테이블의 구조만 복사되고 데이터값은 복사되지 x

-- employee 테이블에서 사번, 직원명, 급여, 연봉만 저장하는 테이블 생성
CREATE TABLE employee_copy 
AS SELECT emp_id 사번, emp_name 직원명, format(salary,0) 급여, format((salary + salary*ifnull(bonus,0))*12,0) 연봉 FROM kh.employee;
SELECT * FROM employee_copy;
