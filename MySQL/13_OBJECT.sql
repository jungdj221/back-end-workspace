...
/*
    ...
    - 정확한 데이터 분석에 기반을 두지 않은 무분별한 인덱스는 오히려 성능이 떨어진다
*/
...
/*
	프로시저(procedure)
    - SQL 문을 저장하여 필요할 때마다 다시 입력할 필요 없이 간단하게 호출해서 사용 가능하게하는 코드 블록
...
/*
    ...
    DELITIMER ;
*/
-- 사번을 입력받아 해당 사원의 연봉을 계싼하고 리턴하는 함수 생성
DELIMITER //
...
-- 사원의 사번, 사원명,급여, 여봉 조회 (정렬을 연봉 높은 순)

SELECT emp_id, salary, salary_calc(emp_id)'연봉'
FROM employee
...
/*
	...
	ex) 
    - 회원탈퇴시 기존 회원테이블에서 데이터 DELETE 후 곧바로 
      탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리
	- 신고횟수가 일정 수를 넘었을때 해당 회원을 블랙리스트로 처리
    - 입출고에 대한 데이터가 기록(INSERT) 될 때마다 해당 삼풍에 대한 재고수량을
      매번 수정(UPDATE) 해야 되는 경우
...
-- employee 테이블에 새로운 행이 INSERT 될때 마다 '신입사원이 입사했습니다'라는
-- 메시지를 log테이블에 자동으로 INSERT 되는 트리거 생성
DELIMITER //
...
DELIMITER ;
-- employee 테이블에서 부서코드가 D6인 부서코드를 D3으로 변경
UPDATE employee
SET dept_code = 'D3'
WHERE dept_code = 'D6';
...
CREATE TABLE tb_product(
	pcode INT AUTO_INCREMENT PRIMARY KEY, -- 상품변호
...
-- 2. 상품 입/출고를 상세 이력 테이블 생성 (tb_prodetail)
/*
	컬럼 : dcode / 기본키
		  pcode / 외래키
          pdate / DATE/ 기본값 현재날짜
          amount / INT / NOt NULL
          status / ChAR(2) / 입고, 출고 둘 중 선택
*/
...
-- 1번 상품이 오늘날짜로 10개를 입고 (tb_prodetail)
INSERT INTO tb_prodetail(pcode, amount, status)
VALUES(1, 10, '입고');
-- 1번상품의 재고수량 10 증가 (tb_product)
UPDATE tb_product
SET stock = stock + 10
WHERE pcode = 1;
...
/*
    ...
    tb-product 테이블에 매번 자동으로 재고수량 UDPATE 되게끔 트리거 정의
	
    트리거명 : trg_03, if문 사용 가능
    IF 조건
    THEN SQl문
    END IF;
*/
...
BEGIN
	IF NEW.status = '입고'
    THEN UPDATE tb_product  SET stock = stock + NEW.amount WHERE pcode = NEW.pcode;
	ELSE UPDATE tb_product SET stock = stock - NEW.amount WHERE pcode = NEW.pcode;
	END IF; 
END //
DELIMITER ;
