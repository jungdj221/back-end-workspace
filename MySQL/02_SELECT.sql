/* 
	- 데이터(data) : 필요에 의해 수집했지만 아직 특정 목적을 위해 정제하지 않은 값
	 vs 정보(info) : 수집한 데이터를 어떠한목적을 위해 분석하거나 가공하여 새로운 의미 부여
    ...
    - Relational(관계형) 데이터베이스
        ...
        3. 데이터의 중복을 최소화할 수 있으며 사용하기 편리하고 
		   데이터의 무결성, 트랜잭션 처리 등 데이터베아스 관리 시스템으로 뛰어난 성능을 보여준다.
...
/*
	...
	- 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 포함될 경우, 반드시 ""사용.
*/
-- employee 테이블에서 직원명 (emp_name),  직원 연봉 조회
...
FROM employee
WHERE dept_code = "D1";
-- 2. employee에서 부서크드가 'D1'이 아닌 사원들의 사번(emp_id), 사원명(emp_name), 부서코드(dept_code) 조회.
...
-- 4. 재직중(ent_yn 칼럼값이 'N')인 사원들의 사본, 사원명, 입사일(hire_date) 조회
...
/*
	LIKE
    - 비교하고자하는 컬럼값이 내가 제시한 특정 채턴에 만족될 겨우 조회
    
    비교대상컬럼 LIKE '특정패턴'
    -특정패턴에는 '%', '_'를 와일드카드 사용
    
    '%' : 0글자 이상
    비교대상컬럼 LIKE '문자%' => 비교대상값이 문자로 "시작"되는걸 조회
    ...
    비교대상컬럼 LIKE '_문자_' => 비교대상컬럼값에 문자앞,뒤에 무조건 한글자가 올 경우 조회
*/
-- 사원들중 성이 전씨인 사원들의 사원명, 급여, 입사일을 조회
SELECT emp_name, salary, hire_date
FROM employee
WHERE emp_name LIKE '전%';

-- 이름 중에 하가 포함된 사원들의 사원명, 주민번호(emp_no), 전화번호(phone) 조회
...
-- ex) sim_bs@kh.or.kr, sun_di@kh.pr.kr
-- ESCAPE OPTION : 나만의 와일드카드를 만들어서 사용
select emp_id, emp_name, email
FROM employee
WHERE email LIKE '___$_%' ESCAPE '$'; /*escape 옆의 옵션은 본인이 임의로 만듬*/
...
/*
	IS NULL / IS NOT NULL
    - 컬람값에 NULL이 있을 경우 NULL 값 비교에 사용되는 연산자
*/
-- 보너스를 받지 않는 사원(bonus 값이 null)
-- 사번, 이름, 급여, 보너스 조회
