...
public class C_Collect {
	...
	public static void main(String[] args) {
		...
		// List 컬렉션으로 수집 -> 수학점수가 80점이상인 사람들만
		List<Student> list = students.stream().filter(score -> score.getMath() >= 80)
						 .collect(Collectors.toCollection(()-> new ArrayList<>()));
		
		System.out.println(list);
		
		// Set 컬렉션 수집 -> 중학생인 사람들만 (14~16살), HashSet
		Set<Student> set = students.stream().filter(age -> age.getAge() >= 14 && age.getAge() <=16)
