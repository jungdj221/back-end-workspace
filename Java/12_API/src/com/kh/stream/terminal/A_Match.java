...
public class A_Match {
	...
	public void method1() {
		...
		System.out.println("3의 배수값이 하나라도 있는가 : " + result);
		
		result = Arrays.stream(values).noneMatch(value -> value % 5 == 0);
		System.out.println("모두가 5의 배수값가 아닌가 : " + result);
	...
	public void method2() {
		...
		// 나이가 25살 이상인 학생들이 모두 남자인지를 확인
		boolean result =  students.stream().allMatch(age -> age.getAge() >= 25&& age.getGender().equals("남자"));
		// 나이관련은 filter로 처리해도 됨.
		System.out.println("25살 이상이 전부 남자인가 : " + result);
		// 남학생 중에 평균이 70점 이상인 학생이 한 명이라도 존재하는지 확인
		result = students.stream()
				.filter(student -> student.getGender().equals("남자"))
				.anyMatch(student -> (student.getMath()+student.getEnglish())/2 >= 70 );
		System.out.println("남자학생중 평균 70이상이 존재? : " + result);
	}
}
