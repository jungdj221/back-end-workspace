 ...
 * - 모든 클래스는 하나의 패키지에 속하며, 패키지가 선언되지 않는 클래스는 자동적으로 이름 없는 패키지에(default)에 속하게 된다.
 * */
package com.kh;
 ...
 * */
public class Hello { // 클래스(변수(속성)와 함수(기능))명은 클래스 파일명과 동일해야 한다
	...
	// 실행 메소드 (main method) : 자바 프로그램 실행 시 가장 먼저 호출
	// 모든 클래스가 main method를 가지고 있어햐 하는 건 아님.
	// 하나의 Java 애플리케이션에는 main method를 포함한 클래스가 반드시 하나는 있어야 한다
 ...
 * - 클래스명 : 대문자로 시작, 여러 개의 단어가 있을 때는 단어 앞자리마다 대분자로 표기.
 * - 메소드명 : 소문자로 시작, 여러 개의 단어가 있을 때는 단어 앞자리마다 대문자로 표기.
 * 
 * * 자바의 특징
 * - 객체지향 프로그래밍(0Object_Oriented Programming, OOP)언어다.
